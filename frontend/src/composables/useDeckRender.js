...
import { Deck, OrbitView, FlyToInterpolator } from '@deck.gl/core';
...
export function useDeckRender() {
    ...
    const initGraph = (containerRef, rawNodes, rawEdges, initialActiveNode) => {
        ...
        // Ponteiro O(1) de acesso direto (Limpo)
        ...
        physicsWorker.onmessage = (event) => {
            ...
            if (type === 'TICK') {
                ...
                for (let i = 0; i < positions.length; i++) posMap.set(positions[i].id, positions[i]);
                
                for (let i = 0; i < currentNodes.length; i++) {
                    const id = currentNodes[i].id;
                    const p = posMap.get(id);
                    if (p) {
                        currentNodes[i].x = p.x;
                        currentNodes[i].y = p.y;
                        currentNodes[i].z = p.z;
                    }
                }
    ...
    const updateGraph = (rawNodes, rawEdges) => {
        ...
        // Re-sincroniza referências de objetos para renderização do Deck.gl
        const nodeMap = new Map();
        currentNodes.forEach(n => nodeMap.set(n.id, n));
    ...
    const updateLayers = (tickCounter = null) => {
        ...
        const layers = [
            ...
            new TextLayer({
                ...
                getText: node => {
                    const isElite = (node.degree > 35) || (node['document-type'] === 'source') || (node['document-type'] === 'system');
                    const isImportant = (node.degree > 12);
                    ...
                    // Lógica de Radar de Aproximação (LOD):
                    // 1. Elite ou Selecionado: Surgem em Zoom Médio (-1.5)
                    if ((isElite || isHovered || isSelected) && zoom > -1.5) {
                        const name = node.name || 'Nó';
                        return name.length > 20 ? name.substring(0, 18) + '..' : name;
                    }
                    
                    // 2. Memórias e Nós Importantes: Surgem em Zoom de Identificação (> 0.8)
                    if ((isMemory || isImportant) && zoom > 0.8) {
                        const name = node.name || 'Dado';
                        return name.length > 18 ? name.substring(0, 16) + '..' : name;
                    }

                    // 3. Detalhes (Resto): Apenas com Zoom Close-up (> 2.8)
                    if (zoom > 2.8) {
                ...
                getSize: node => {
                    const isElite = (node.degree > 35);
                    const isMemory = node['document-type'] === 'memory';
                    if (isElite) return 14;
                    if (isMemory) return 10;
                    return 9;
                },
                getColor: node => {
                    const isMemory = node['document-type'] === 'memory';
                    ...
                    if (isElite || store.hoveredNodeId === node.id || store.selectedNodeId === node.id) {
                        alpha = Math.max(0, Math.min(255, (zoom + 1.8) * 300));
                    } else if (isMemory || isImportant) {
                        alpha = Math.max(0, Math.min(255, (zoom - 0.5) * 300));
                    } else {
                        alpha = Math.max(0, Math.min(255, (zoom - 2.5) * 300));
                    }

                    if (isMemory) return [244, 114, 182, alpha];
                    if (isElite) return [255, 255, 255, alpha];
                    return [255, 255, 255, alpha * 0.6]; 
                },
                getAngle: 0,
                getTextAnchor: 'start',
                ...
                fontFamily: 'Inter, sans-serif',
                fontWeight: 600,
                outlineWidth: 1, // Reduzido para ficar mais fino
            ...
            new LineLayer({
                ...
                getColor: link => {
                    ...
                    // Arestas clicadas = branco, hover = dourado
                    if (clLinks.has(`${s}-${t}`) || clLinks.has(`${t}-${s}`)) return [255, 255, 255, 180]; 
                    if (hlLinks.has(`${s}-${t}`) || hlLinks.has(`${t}-${s}`)) return [252, 211, 77, 150];  
                    // Base: Teal ESCURO quase invisível — nós é que dominam
                    return [0, 180, 200, 10]; 
                },
                getWidth: link => {
                    const weight = link.weight || 1;
                    return Math.min(0.3 + (weight * 0.15), 1.0); // Ultra-finas
                },
                updateTriggers: {
                    getColor: [store.clickedNodeLinks.size, store.highlightedLinks.size],
            ...
            new ScatterplotLayer({
                ...
                getFillColor: node => {
                    ...
                    // Prioridade 1: Cor da Comunidade (Cluster Semântico)
                    const cCol = getCommunityColor(node.community);
                    if (cCol) return [...cCol, 220];

                    // Prioridade 2: Cor por Tipo de Documento
                    const type = node['document-type'] || 'chunk';
                    return colors[type] ? [...colors[type], 220] : [120, 120, 120, 220];
                },
                getRadius: node => getRadius(node),
                radiusUnits: 'pixels',    
                radiusMinPixels: 0.5,       
